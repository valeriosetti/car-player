...
void lv_table_set_active_cell(lv_obj_t * obj, uint16_t row, uint16_t col)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
...
void lv_table_set_selected_cell(lv_obj_t * obj, uint16_t row, uint16_t col, bool send_event)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    ...
    if(table->col_sel != col || table->row_sel != row) {
        table->col_sel = (col >= table->col_cnt) ? (table->col_cnt - 1) : col;
        table->row_sel = (row >= table->row_cnt) ? (table->row_cnt - 1) : row;

        lv_obj_invalidate(obj);

        scroll_to_selected_cell(obj);

        if (send_event) lv_obj_send_event(obj, LV_EVENT_VALUE_CHANGED, NULL);
    }
}
...
void lv_table_get_active_cell(lv_obj_t * obj, uint32_t * row, uint32_t * col)
{
    lv_table_t * table = (lv_table_t *)obj;
    *row = table->row_act;
    *col = table->col_act;
...
static void lv_table_event(const lv_obj_class_t * class_p, lv_event_t * e)
{
    ...
    if(code == LV_EVENT_STYLE_CHANGED) {
        ...
        if(table->col_act != LV_TABLE_CELL_NONE && table->row_act != LV_TABLE_CELL_NONE && scroll_obj == NULL) {
            table->col_sel = table->col_act;
            table->row_sel = table->row_act;
